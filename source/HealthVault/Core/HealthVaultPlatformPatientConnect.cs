...
namespace Microsoft.Health.PlatformPrimitives
{
    ...
    public class HealthVaultPlatformPatientConnect
    {
        ...
        private static void WritePackageBlobRefUrls(
            List<Uri> packageContentStreamedBlobRefUrls, XmlWriter writer)
        {
            if (packageContentStreamedBlobRefUrls.Count > 0)
            {
                writer.WriteStartElement("streamed-package-blobs");
            }

            for (int i = 0; i < packageContentStreamedBlobRefUrls.Count; ++i)
            {
                writer.WriteElementString(
                    "blob-in-package-ref-url",
                    packageContentStreamedBlobRefUrls[i].OriginalString);
            }

            if (packageContentStreamedBlobRefUrls.Count > 0)
            {
                writer.WriteEndElement(); //</streamed-package-blobs>
            }
        }
