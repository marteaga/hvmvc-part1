...
namespace Microsoft.Health
{
    ...
    public class HealthRecordExporter
    {
        ...
        private XslCompiledTransform GetTransform(string tag)
        {
            ...
            transform.Load(url.OriginalString, settings, new XmlUrlResolver());
            return transform;
        }
        ...
        protected virtual string TransformItemXml(string healthRecordItemsXml)
        {
            ...
            if (_transform != null)
            {
                ...
                StringReader txtReader = new StringReader(healthRecordItemsXml);
                using (XmlReader reader = XmlReader.Create(txtReader, readerSettings))
                {
                    ...
                    using (XmlWriter writer = XmlWriter.Create(resultXml, settings))
                    {
                        _transform.Transform(reader, _transformArgumentList, writer);
